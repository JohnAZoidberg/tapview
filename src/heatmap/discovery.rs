...
/// Payload length of the heatmap burst report (Feature report 0x41), from
/// the device's report descriptor. `burst_read` needs it to size each
/// GetFeature.
pub fn burst_report_length(layout: &crate::hid::ReportLayout) -> io::Result<usize> {
    match layout.report_bytes(crate::hid::ReportKind::Feature, 0x41) {
        0 => Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "could not find Feature report 0x41 in HID descriptor",
        )),
        n => Ok(n),
    }
}
...
pub fn determine_burst_report_length(hidraw_path: &Path) -> io::Result<usize> {
    ...
    burst_report_length(&crate::hid::ReportLayout::parse(&desc))
}
